...
from presentations.design_tokens import COLORS, FONTS, SIZES, LAYOUT
...
def build():
    ...
    # Dark background
    add_rect(slide, 0, 0, LAYOUT["slide_w_in"], LAYOUT["slide_h_in"], COLORS["bg_dark"])

    # Title
    add_text_box(slide, "Step 2 — Icon Glyph Proof Sheet",
                 LAYOUT["margin_x_in"], 0.20,
                 12.0, 0.60,
                 font_name=FONTS["header_serif"],
                 size=SIZES["section_header"],
                 color=COLORS["ink_on_dark"],
                 bold=True)

    add_text_box(slide, "Unicode Approach B · every icon rendered in Segoe UI Symbol",
                 LAYOUT["margin_x_in"], 0.72,
                 12.0, 0.35,
                 font_name=FONTS["body_sans"],
                 size=SIZES["caption"],
                 color=COLORS["ink_on_dark_muted"])

    # Subtle rule
    rule = slide.shapes.add_shape(1,
        Inches(LAYOUT["margin_x_in"]), Inches(1.12),
        Inches(LAYOUT["slide_w_in"] - 2 * LAYOUT["margin_x_in"]), Inches(0.012))
    rule.fill.solid()
    rule.fill.fore_color.rgb = rgb(COLORS["rule_on_dark"])
    rule.line.fill.background()

    # Layout icons in a grid: 4 columns
    names = sorted(ICON_MAP.keys())
    col_count  = 4
    col_w      = (LAYOUT["slide_w_in"] - 2 * LAYOUT["margin_x_in"]) / col_count
    start_y    = 1.25
    row_h      = 0.40
    ...
    for idx, name in enumerate(names[:col_count * max_rows]):
        col = idx % col_count
        row = idx // col_count
        x = LAYOUT["margin_x_in"] + col * col_w
        y = start_y + row * row_h
        ...
        gr.font.size = Pt(16)
        gr.font.color.rgb = rgb(COLORS["accent_coral"])
